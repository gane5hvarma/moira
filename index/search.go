...
func (index *Index) SearchTriggers(filterTags []string, searchString string, onlyErrors bool, page int64, size int64) (highlights moira.Highlights, total int64, err error) {
	if !index.checkIfIndexIsReady() {
		return make([]moira.Highlight, 0), 0, fmt.Errorf("index is not ready, please try later")
	}
