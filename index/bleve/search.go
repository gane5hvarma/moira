...
// Relevance (asc)
// Trigger.Name (asc)
func (index *TriggerIndex) Search(filterTags []string, searchString string, onlyErrors bool, page int64, size int64) (highlights moira.Highlights, total int64, err error) {
	if size < 0 {
		page = 0
	...
	for _, result := range searchResult.Hits {
		highlight := moira.Highlight{
			TriggerID: result.ID,
		}
		if triggerName, ok := result.Fragments[mapping.TriggerName.String()]; ok {
			highlight.Name = triggerName
		}
		if triggerDesc, ok := result.Fragments[mapping.TriggerDesc.String()]; ok {
			highlight.Desc = triggerDesc
		}
